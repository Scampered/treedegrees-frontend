    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 relative">
        ...
        {error && !loading && (
          <div className="absolute inset-0 z-20 flex items-center justify-center">
            <div className="glass rounded-xl p-6 max-w-sm text-center">
              ...
              <p className="text-forest-500 text-sm mt-1">Add connections to see your map grow!</p>
        ...
        <MapContainer
          ...
          worldCopyJump
        >
          {/* Carto dark tiles — English labels, no API key needed */}
          <TileLayer
          ...
          {filteredNodes.map(node => {
            ...
            return (
              <CircleMarker
                ...
                {...style}
                eventHandlers={{ click: () => setFlyTo([node.latitude, node.longitude]) }}
              >
                <Popup>
                  <div style={{ background: '#082208', color: '#f0faf0', borderRadius: 8, padding: 10, minWidth: 150, fontFamily: 'DM Sans, sans-serif' }}>
                    <div style={{ marginBottom: 6 }}>
                      <span style={{ fontSize: 10, padding: '2px 6px', borderRadius: 9999, background: '#196219', color: '#80d580', border: '1px solid #2d9e2d' }}>{text}</span>
                      {node.locationPrivacy && !isMe && <span style={{ fontSize: 10, marginLeft: 4, color: '#4d7a4d' }}>📍 Approx.</span>}
                    </div>
                    <p style={{ fontWeight: 600, fontSize: 13, margin: '4px 0 2px' }}>{node.fullName}</p>
                    <p style={{ fontSize: 11, color: '#4dba4d', margin: 0 }}>{node.city}, {node.country}</p>
                    {node.dailyNote && !isMe && (
                      <p style={{ fontSize: 11, color: '#80d580', marginTop: 8, fontStyle: 'italic', borderTop: '1px solid #196219', paddingTop: 6 }}>
                        "{node.dailyNote}"
                      </p>
                    )}
                  </div>
