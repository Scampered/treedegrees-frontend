...
const JOB_LABELS = {
  courier:'Courier', writer:'Writer', seed_broker:'Seed Broker',
...
function buildNodeIcon(degree, hasNote, isHidden, mood, seeds) {
  if (isHidden) {
    return L.divIcon({
      ...
      html: `<div style="width:14px;height:14px;border-radius:50%;background:#1a1a1a;border:2px dashed #555;display:flex;align-items:center;justify-content:center;font-size:9px;color:#777;font-weight:bold;">?</div>`,
    })
  }
  const { bg, border, size } = DEGREE_COLORS[degree] || DEGREE_COLORS[3]
  const half = Math.round(size / 2)
  const badge = hasNote
    ? `<div style="position:absolute;top:-4px;right:-4px;width:10px;height:10px;border-radius:50%;background:white;border:2px solid #0d2b0d;box-shadow:0 0 4px rgba(0,0,0,0.6);z-index:10;"></div>`
    : ''
  const seedsPill = (seeds !== null && seeds !== undefined)
    ? `<div style="position:absolute;bottom:-15px;left:50%;transform:translateX(-50%);white-space:nowrap;font-size:9px;font-weight:700;font-family:monospace;background:rgba(0,0,0,0.8);color:#4ade80;padding:1px 5px;border-radius:8px;border:1px solid rgba(74,186,74,0.35);pointer-events:none;z-index:20;">🌱${seeds}</div>`
    : ''
  const extraBottom = (seeds !== null && seeds !== undefined) ? 17 : 0
  const totalH = size + extraBottom
  if (mood) {
    return L.divIcon({
      className: '', iconSize: [size,totalH], iconAnchor: [half,half],
      popupAnchor: [0,-half-4], tooltipAnchor: [0,-half-4],
      html: `<div style="position:relative;width:${size}px;height:${totalH}px;"><div style="width:${size}px;height:${size}px;border-radius:50%;background:${bg};border:${degree===0?3:2}px solid ${border};box-shadow:0 0 ${degree===0?10:5}px ${bg}88;display:flex;align-items:center;justify-content:center;font-size:${Math.round(size*0.52)}px;line-height:1;">${mood}</div>${badge}${seedsPill}</div>`,
    })
  }
  return L.divIcon({
    className: '', iconSize: [size,totalH], iconAnchor: [half,half],
    popupAnchor: [0,-half-4], tooltipAnchor: [0,-half-4],
    html: `<div style="position:relative;width:${size}px;height:${totalH}px;"><div style="width:${size}px;height:${size}px;border-radius:50%;background:${bg};border:${degree===0?3:2}px solid ${border};box-shadow:0 0 ${degree===0?10:5}px ${bg}88;"></div>${badge}${seedsPill}</div>`,
  })
}
...
function buildVehicleIcon(tier) {
  ...
  return L.divIcon({
    ...
    html: `<div style="font-size:${size}px;line-height:1;filter:drop-shadow(0 2px 4px rgba(0,0,0,0.8));pointer-events:none;">${emoji}</div>`,
  })
}
...
function buildLabelIcon(name, zoom) {
  ...
  return L.divIcon({
    ...
    html: `<div style="font-family:Dosis,sans-serif;font-size:${fs}px;font-weight:700;color:#e0ffe0;white-space:nowrap;pointer-events:none;letter-spacing:0.03em;text-shadow:0 0 6px #000,0 0 12px #000,1px 1px 0 #000,-1px -1px 0 #000;">${name}</div>`,
  })
}
...
function FlyToTarget({ lat, lng }) {
  const map = useMap()
  useEffect(() => { if (lat && lng) map.flyTo([lat,lng], 8, { animate:true, duration:1.5 }) }, []) // eslint-disable-line
  return null
}

function FlyToOnOpen({ lat, lng }) {
  const map = useMap()
  useEffect(() => { map.flyTo([lat,lng], Math.max(map.getZoom(),6), { animate:true, duration:1.2 }) }, []) // eslint-disable-line
  return null
}
...
// ── MapSendModal stub (imported elsewhere normally) ───────────────────────────
function MapSendModal({ friends, streaks, groups, onSend, onClose }) {
  return (
    <div className="bg-forest-950 border border-forest-700 rounded-2xl p-6 w-full max-w-sm">
      <div className="flex items-center justify-between mb-4">
        <h3 className="font-display text-forest-100 text-lg">✉️ Send Letter</h3>
        <button onClick={onClose} className="text-forest-500 hover:text-forest-300 text-xl">✕</button>
      </div>
      <p className="text-forest-500 text-sm">Use the Letters page to send letters.</p>
      <button onClick={onClose} className="mt-4 w-full py-2 bg-forest-700 hover:bg-forest-600 text-forest-100 rounded-xl text-sm transition-colors">
        Close
      </button>
    </div>
  )
...
export default function MapPage() {
  ...
  const filteredNodes = (mapData?.nodes || []).filter(n => {
    if (!n.latitude || !n.longitude || isNaN(Number(n.latitude)) || isNaN(Number(n.longitude))) return false
  ...
  const mapCSS = `
    .leaflet-popup-content-wrapper {
      background:#0d2b0d!important;border:1px solid #2d6a2d!important;
      border-radius:12px!important;box-shadow:0 4px 20px rgba(0,0,0,0.6)!important;padding:0!important;
    }
    .leaflet-popup-content { margin:0!important; }
    .leaflet-popup-tip { background:#0d2b0d!important; }
    .note-tip .leaflet-tooltip {
      background:rgba(8,34,8,0.92);border:1px solid #2d6a2d;border-radius:8px;
      color:#c0f0c0;font-family:Dosis,sans-serif;padding:6px 10px;font-size:12px;
    }
      ...
      <div className="flex-1 relative min-h-0">
        ...
        <MapContainer center={[25,30]} zoom={4} minZoom={2} maxZoom={18}
          ...
          {filteredEdges.map((edge, i) => {
            ...
            if (!src?.latitude || !tgt?.latitude || isNaN(Number(src.latitude)) || isNaN(Number(tgt.latitude))) return null
            const style = (edge.isPrivate || edge.isHidden) ? PRIVATE_LINE : (LINE_STYLES[edge.degree] || LINE_STYLES[3])
            return (
              <Polyline key={i}
                positions={[[Number(src.latitude), Number(src.longitude)], [Number(tgt.latitude), Number(tgt.longitude)]]}
                pathOptions={style} />
            )
          })}
          ...
          {filteredNodes.map(node => {
            ...
            return (
              <Marker key={node.id} position={[Number(node.latitude), Number(node.longitude)]}
                ...
                <Popup autoPan={false} closeButton={false} eventHandlers={{ add:()=>{} }}>
                  <FlyToOnOpen lat={Number(node.latitude)} lng={Number(node.longitude)} />
                    ...
                    {node.hiddenByPrivateLink ? (
                      ...
                      <>
                        ...
                        {node.degree === 1 && node.jobRole && (
                          <p style={{ fontSize:11, color:'#80a080', margin:'0 0 6px', display:'flex', alignItems:'center', gap:4 }}>
                            <span>{JOB_EMOJIS[node.jobRole]}</span>
                            <span>{JOB_LABELS[node.jobRole] || node.jobRole}</span>
                          </p>
                        )}
                        {node.degree === 1 && (
                          <button onClick={() => setProfileNode(node)}
                            style={{
          ...
          {showGroups && groupMapData.map(group => {
            ...
            return (
              <Polygon key={group.id} positions={positions}
                pathOptions={{ color:group.color, fillColor:group.color, fillOpacity:0.12, weight:1.5, opacity:0.5 }} />
            )
          })}

          {groupTransitPos
            .filter(v => v.currentLat != null && !isNaN(v.currentLat))
            .map(v => (
              <Marker key={v.id} position={[v.currentLat, v.currentLon]}
        ...
        {showSend && (
          <div className="absolute inset-0 z-[9999] flex items-end sm:items-center justify-center p-4 bg-black/70">
            <MapSendModal friends={friends} streaks={streaks} groups={mapGroups}
                ...
                {profileNode.jobRole && (
                  <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:10 }}>
                    ...
                    <span style={{ fontSize:12, color:'#80a080' }}>
                      {JOB_LABELS[profileNode.jobRole] || profileNode.jobRole}
                    </span>
                  </div>
                )}
                ...
                <div style={{ display:'flex', gap:8 }}>
                  <button
                    ...
                    style={{
                      ...
                      background:'rgba(45,158,45,0.1)', border:'1px solid #2d6a2d',
                      color:'#6aaa6a', cursor:'pointer',
                    }}>
                    👤 Profile
                  </button>
                  <button
                    ...
                    style={{
                      flex:1, padding:'7px 0', borderRadius:10, fontSize:12, fontWeight:600,
                      background:'#196219', border:'1px solid #2d9e2d',
                      color:'#80d580', cursor:'pointer',
                    }}>
                    ✉️ Write letter
                  </button>
                </div>
